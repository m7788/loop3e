...
Root Token Budget 不适用于 spec 或 plan 完整性。spec 的结构、深度和用户确认流程由 `superpowers:brainstorming` 统一决定；plan 的结构、深度、任务颗粒度和执行顺序由 `superpowers:writing-plans` 统一决定；预算只约束 checkpoint、handoff、report 读取和重复回显。

Root 给 Generator / Evaluator 的 handoff 只传 artifact 路径、当前运行目录和短 phase brief；不要粘贴完整 spec、plan、diff、report。子 agent 自己读取 artifact，并把结构化结论写回 `.loop3e/runs/<run_id>/`。
...
* active goal 已包含当前 spec 绝对路径：复用并写入当前运行目录的 `run_log.md`。
...
* mloop 不替代 `superpowers:brainstorming` 的 spec 结构。Root 必须在 `superpowers:brainstorming` 流程内纳入用户明确要求、Root 推断假设、待用户确认、用户已接受的降级；Root 不能把推断假设当作用户授权。
* 涉及用户可见页面、交互流程、导航、弹窗、表格列或空态时，Root 必须让 `superpowers:brainstorming` 覆盖产品/交互设计确认。设计确认只描述入口、信息位置、显示/空态、是否改变页面结构和用户需确认的取舍；有多个合理 UX 方案或会改变用户工作流时先 `ASK_USER`。
...
Generator plan 必须以 `Gate Summary` 开头，覆盖 scope、files、tests、risks 和 acceptance mapping。`Gate Summary` 只是给 Root/Evaluator 的索引，不替代完整 plan。

Root 默认只读 plan 前 80 行做 gate；只有 summary 缺失、自相矛盾或出现阻塞问题时才读完整 plan。

不得为了 token budget 压缩必要计划内容。Evaluator 做 package review 时必须按需读取完整 plan。

### 4.6 Evaluator 新鲜度守卫

Root 在派发 `loop_evaluator` 前必须覆盖当前运行目录的 `verdict.json` 为 `PENDING`，并覆盖当前运行目录的 `evaluator_report.md` 为 pending note，避免复用旧 run 的 PASS verdict。
...
只有本轮新鲜的 Evaluator `PASS` verdict 才能进入 Root 产品验收。
...
| Plan 执行 | `superpowers:subagent-driven-development` / `superpowers:executing-plans` | 当前运行目录的 `package_review.json` 为 `APPROVED` 之后二选一；当前运行目录的 `package_review.json` 为 `APPROVED` 之前不得加载执行类 skill |
...
IMPLEMENT_MODE 的执行拆分由 Generator 决定。若 approved plan 中存在 2 个以上文件所有权清晰、可独立验证、不会编辑同一批文件的任务，满足并行条件时 Generator 默认并行，使用 `superpowers:subagent-driven-development`；不并行必须在 generator_report 说明理由；否则使用 `superpowers:executing-plans` 直接执行。并行前 Generator 必须写紧凑 execution split，列明每个子任务的 files/checks/交付物，并负责最终集成、冲突处理和验证。
...
Root 启动 mloop 时必须把当前工作目录绝对路径记录为 `loop_root`。每次 `$mloop` 启动都创建新的 `run_id`，写入 `.loop3e/current_run.txt`，并把本轮 mutable artifacts 放入 `.loop3e/runs/<run_id>/`。`.loop3e/`、`docs/superpowers/specs/`、`docs/superpowers/plans/`、Evaluator freshness guard 和最终 artifact 读取都必须基于这个 `loop_root`。

不得用 git top-level 或父 workspace 替代 `loop_root`。当测试项目位于大仓库或 ignored workspace 的子目录时，mloop 的运行根仍然是用户启动 Codex 的当前项目目录。

Root 给 Generator / Evaluator 的 handoff 必须包含 `loop_root`、当前运行目录和该根下的绝对 artifact 路径。Generator / Evaluator 只能写入这个 `loop_root` 下的 `.loop3e/` 和 `docs/superpowers/`。

Root、Generator、Evaluator 只能使用当前运行目录里的 package review、report、verdict 和 run_log。根目录旧 `verdict.json`、`generator_report.md`、`evaluator_report.md`、`package_review.json`、`run_log.md` 不得参与门禁判断；若存在，只能视为历史兼容残留。
...
* 在写 spec 前必须调用 `superpowers:brainstorming` 进行需求澄清和设计确认。
* 产出 Superpowers spec / design。
* Root spec 写需求、设计和验收，不写具体实现选择。具体库、API、算法、数据结构、测试实现由 `loop_generator` 在 plan 中提出。
...
| 文件                               | 作用                                 |
| -------------------------------- | ---------------------------------- |
| `.loop3e/current_run.txt`          | 当前 run id                          |
| `.loop3e/current_spec.txt`         | 当前 Superpowers spec 路径             |
| `.loop3e/current_plan.txt`         | 当前 Superpowers plan 路径             |
| `.loop3e/runs/<run_id>/current_spec.txt` | 本轮 spec 指针                    |
| `.loop3e/runs/<run_id>/current_plan.txt` | 本轮 plan 指针                    |
| `.loop3e/runs/<run_id>/package_review.json` | Evaluator 对 spec+plan package 的评审结果 |
| `.loop3e/runs/<run_id>/generator_report.md` | Generator 实现和测试报告                  |
| `.loop3e/runs/<run_id>/verdict.json` | Evaluator 最终验收结论                   |
| `.loop3e/runs/<run_id>/evaluator_report.md` | Evaluator 详细验收报告                   |
| `.loop3e/runs/<run_id>/run_log.md` | 本轮调度记录                               |

每次 `$mloop` 启动都必须创建新的 run 目录，旧 run 保留用于追溯，不要求用户手工清理。默认不要创建更多 `.loop3e` 过程文件。spec / acceptance / implementation plan 已由 `docs/superpowers/` 承载；经验、演进建议、规则更新建议和回归候选默认写进最终回复。只有用户明确要求持久沉淀时，才另建文档。
...
Generator 必须读取当前运行目录的 `package_review.json`。只有 `status=APPROVED` 才能进入实现；`REQUEST_CHANGES` 只能回到 plan 修订；`SPEC_ISSUE`、`ASK_USER`、`STOP` 必须停下交给 Root 路由。
...
- .loop3e/current_plan.txt（兼容指针）
- 当前运行目录的 current_plan.txt
- 当前运行目录的 generator_report.md
- 业务代码
- 测试代码
- 必要文档
...
当当前运行目录的 package_review.json status 为 REQUEST_CHANGES：
...
当当前运行目录的 package_review.json status 为 APPROVED：
1. 读取 approved plan。
2. 按 plan 最小范围实现。
3. 补充或更新测试。
4. 运行验证命令。
5. 输出当前运行目录的 generator_report.md。
...
当 Evaluator 返回 FAIL：
1. 读取当前运行目录的 verdict.json。
2. 只修 blocking_findings。
3. 不扩大范围。
4. 不新增无关重构。
5. 更新当前运行目录的 generator_report.md。
6. 重新运行相关测试。
"""
```
...
7. 输出当前运行目录的 verdict.json 和 evaluator_report.md。

你可以写入：
- 当前运行目录的 package_review.json
- 当前运行目录的 verdict.json
- 当前运行目录的 evaluator_report.md
...
6. 不要求无关测试。
7. 不阻塞非关键优化项。
8. 输出当前运行目录的 package_review.json。
...
3. 读取当前运行目录的 package_review.json。
4. 读取当前运行目录的 generator_report.md。
5. 检查 git diff、测试代码、测试日志、CI 结果。
6. 输出当前运行目录的 verdict.json 和 evaluator_report.md。
...
.loop3e/current_run.txt
.loop3e/runs/<run_id>/package_review.json
.loop3e/runs/<run_id>/generator_report.md
.loop3e/runs/<run_id>/verdict.json
.loop3e/runs/<run_id>/evaluator_report.md
.loop3e/runs/<run_id>/run_log.md
```
...
5. Append safety notes to the current run directory `run_log.md`.

## Phase 1: Root creates or updates spec

Root uses this skill as Planner discipline, but does not replace Superpowers brainstorming. Spec structure, depth, and user review follow `superpowers:brainstorming`.
...
4. Include acceptance criteria and Loop3E needs inside the brainstorming/spec flow: assumptions, pending confirmations, accepted downgrades, observable evidence, UI/UX trade-offs when relevant, and cross-system delivery boundaries when relevant.
...
2. Root Codex routes the resulting current run directory `package_review.json`.
...
Only continue if the current run directory `package_review.json` status is APPROVED.
...
6. Write the current run directory `generator_report.md`.
...
派发前，Root 写入 Evaluator 新鲜度守卫：当前运行目录的 `verdict.json` 设为 `PENDING`，当前运行目录的 `evaluator_report.md` 记录最终验收已开始。
...
.loop3e/current_spec.txt
.loop3e/current_plan.txt
current run directory package_review.json
current run directory generator_report.md
git diff
test logs
AGENTS.md
```
...
```text
current run directory verdict.json
current run directory evaluator_report.md
```
...
3. Generator 只修复阻塞问题。
4. Generator updates the current run directory `generator_report.md`.
