...
assert_contains "$SKILL" 'Root spec 只写需求和验收，不写具体实现选择'
assert_contains "$SKILL" 'Root spec 必须区分：用户明确要求、Root 推断假设、待用户确认、用户已接受的降级'
...
assert_contains "$SKILL" '`.loop3e/package_review.json` 为 `APPROVED` 后'
...
assert_contains "$SKILL" '交接只传 `loop_root`、artifact 路径和短 phase brief'
assert_contains "$SKILL" 'Root 写 Evaluator 新鲜度守卫'
assert_contains "$SKILL" '覆盖 `.loop3e/verdict.json` 为 `PENDING`'
...
assert_contains "$SKILL" 'Generator 实现前必须读取 `.loop3e/package_review.json`'
assert_contains "$SKILL" '`PASS` 后 Root 做产品验收和最终总结'
assert_contains "$SKILL" "默认运行产物只保留"
...
assert_contains "$GENERATOR" "先读取 .loop3e/package_review.json"
...
assert_contains "$GENERATOR" "小任务 plan 目标不超过 100 行"
...
assert_contains "$DOC" 'Root 给 Generator / Evaluator 的 handoff 只传 artifact 路径和短 phase brief'
...
assert_contains "$DOC" "Root spec 必须区分用户明确要求、Root 推断假设、待用户确认、用户已接受的降级"
...
assert_contains "$DOC" 'Root 在派发 `loop_evaluator` 前必须覆盖 `.loop3e/verdict.json` 为 `PENDING`'
...
assert_contains "$DOC" '`superpowers:subagent-driven-development` / `superpowers:executing-plans` | `.loop3e/package_review.json` 为 `APPROVED` 之后二选一'
assert_contains "$DOC" 'package_review.json` 为 `APPROVED` 之前不得加载执行类 skill'
...
assert_contains "$DOC" 'Root spec 只写需求和验收，不写具体实现选择'
...
assert_contains "$DOC" 'Generator 必须读取 `.loop3e/package_review.json`'
